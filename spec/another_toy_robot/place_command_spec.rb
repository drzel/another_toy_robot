...
describe PlaceCommand do
  ...
  describe '#execute' do
    context 'when params are valid' do
      let(:command) do
        build :place_command, target: robot, params: %w[0 0 north]
      end
      ...
      context 'when @params[2] is "east"' do
        let(:command) do
          build :place_command, target: robot, params: %w[0 0 east]
        end
      ...
      context 'when @params[2] is "south"' do
        let(:command) do
          build :place_command, target: robot, params: %w[0 0 south]
        end
      ...
      context 'when @params[2] is "west"' do
        let(:command) do
          build :place_command, target: robot, params: %w[0 0 west]
        end
